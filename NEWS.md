...
* Added a `detset_oneof` argument for `discover`/`autodb`, which requires the FD search to only return dependencies where the determinant set contains any of the given vectors of attribute names. Currently, this is only used to check additional conditions under which to skip a dependant, or the entire search, not to prune the search itself.
