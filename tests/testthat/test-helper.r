describe("gen_df", {
  it("can give a table of exact height", {
    forall(
      gen.element(0:6) |>
        gen.and_then(\(n) list(
          gen.pure(n),
  ...
  it("can give a table of exact width", {
    forall(
      gen.element(0:7) |>
        gen.and_then(\(n) list(
          gen.pure(n),
  ...
  it("can generate a tibble without name failure", {
    forall(
...
describe("gen.float_coincide", {
  it("adds offset that disappear in rounding", {
    forall(
      list(
        gen.numeric(c(1, 0)),
...
describe("gen.relation_schema_empty_keys", {
  it("generates valid relation schemas, enough empty keys", {
    forall(
      gen.element(0:4) |>
        gen.and_then(\(from) {
          gen.element(0:from) |>
...
describe("gen.database_schema", {
  it("generates valid database schemas", {
    forall(
      gen.element(c(FALSE, TRUE)) |>
        gen.list(of = 3) |>
...
describe("gen.database_schema_empty_keys", {
  it("generates valid relation schemas, enough empty keys", {
    forall(
      list(
        gen.element(c(FALSE, TRUE)),
...
describe("gen.relation", {
  it("generates valid relations", {
    forall(
      gen.element(c(FALSE, TRUE)) |>
        gen.and_then(\(sek) list(
          gen.pure(sek),
...
describe("gen.database", {
  it("generates valid databases", {
    forall(
      gen.element(c(FALSE, TRUE)) |>
        gen.list(of = 3) |>
        gen.and_then(uncurry(\(sek, san, skp) list(
...
describe("remove_insertion_key_violations", {
