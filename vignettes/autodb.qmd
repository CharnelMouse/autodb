...
This can be saved to a file for passing to Graphviz elsewhere, or can be passed into an R function that renders Graphviz code, such as `grViz` in the `DiagrammeR` package. Here, I make use of Quarto having Graphviz's dot engine available in code chunks, so I can call GraphViz directly after saving the code to a file:

```{r}
#| label: db_plot
write(gv(db), "ChickWeight.gv")
```
...
```{r}
#| include: false
write(gv(schema), "ChickWeight_schema.gv")
```

```{dot}
//| file: ChickWeight_schema.gv
```
...
```{r}
#| include: false
write(gv(autodb(as.data.frame(Titanic))), "Titanic.gv")
```
...
```{r}
#| include: false
write(gv(autodb(as.data.frame(Titanic), exclude = "Freq")), "Titanic_nonum.gv")
```

```{dot}
//| file: Titanic_nonum.gv
```
...
```{r}
#| include: false
write(gv(linked_schema), "ChickWeight_linked.gv")
```

```{dot}
//| file: ChickWeight_linked.gv
```
...
```{r}
#| include: false
write(gv(db2), "ChickWeight_decomposed.gv")
```

```{dot}
//| file: ChickWeight_decomposed.gv
```
...
```{r}
#| include: false
write(gv(nudge_schema), "nudge_schema.gv")
```

```{dot}
//| file: nudge_schema.gv
```
...
```{r}
#| include: false
write(gv(nudge_schema_filtered), "nudge_filtered.gv")
```

```{dot}
//| file: nudge_filtered.gv
```
...
```{r}
#| include: false
write(gv(nudge_schema_final), "nudge_final.gv")
```
...
#| include: false
write(gv(nudge_database_final), "nudge_db_final.gv")
```

```{dot}
//| file: nudge_db_final.gv
```
...
```{r}
#| label: nudge_publication_badfilter_gv
#| include: false
write(gv(nudge_schema_relfiltered), "nudge_relfiltered.gv")
```

```{dot}
//| file: nudge_relfiltered.gv
```

Re-running `autoref` has no effect, because the `es_id` section doesn't contain any key for the `title` section:

```{r}
#| label: nudge_publication_badfilter_autoref
...
```{r}
#| label: nudge_badfilter_example_fds
...
``` r
normalise(example_fds, ensure_lossless = FALSE)
```

```{r nudge_badfilter_example_transitive}
#| include: false
write(gv(normalise(example_fds, ensure_lossless = FALSE)), "nudge_transitive.gv")
```

```{dot}
//| file: nudge_transitive.gv
```
...
#| include: false
write(gv(normalise(example_fds[-2], ensure_lossless = FALSE)), "nudge_nontransitive.gv")
```

```{dot}
//| file: nudge_nontransitive.gv
```
...
discover(
  ...
  normalise() |>
  gv() |>
  write("nudge_approx_cheat.gv")
```

```{dot}
//| file: nudge_approx_cheat.gv
```
...
```{r}
#| include: false
write(gv(nudge_approx_database_schema), "nudge_approx.gv")
```

```{dot}
//| file: nudge_approx.gv
```
...
``` r
reduce(nudge_approx_database_schema, "es_id")
```

```{r nudge_approximate_reduced}
#| include: false
write(gv(reduce(nudge_approx_database_schema, "es_id")), "nudge_approx_reduced.gv")
```

```{dot}
//| file: nudge_approx_reduced.gv
```
...
```{r}
#| include: false
write(gv(avoid_schema), "avoid_schema.gv")
```

```{dot}
//| file: avoid_schema.gv
```
...
```{r}
#| include: false
write(gv(avoid_schema_removed), "avoid_schema_removed.gv")
```

```{dot}
//| file: avoid_schema_removed.gv
```
