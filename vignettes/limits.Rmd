...
In practice, have an attribute's class vary across the relation it belongs to is asking for trouble.

In particular, if it's representing by a factor in one relation, and a non-factor, non-character class in another, where the latter has values not in the former's levels, then merging them will cause issues. This is not unexpected, it's just how coercing on factors works in R.
