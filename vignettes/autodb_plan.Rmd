...
Usually, views are used to allow easy "look-up" of commonly-queried aggregates. Here, we can use them to maintain the connected schema structure.
...
schema2 <- c(
  ...
  database_schema(
    relation_schema(
      list(view = list(attrs_order(schema), list("a"))),
      attrs_order(schema)
    ),
    list()
  )
)
references(schema2) <- c(
  references(schema),
  list(
    list("view", "a", "a", "a"),
    list("view", c("d", "e"), "d_e", c("d", "e"))
  )
)
show(schema2)
```

This example is limited by current restrictions in autodb:

- Virtual relations need not have a key. In this example, the view does not need to explicitly have `a` as a key, since this key can be inferred from the real relations.
- Virtual relations should be more distinguishable from real relations.

Still, this is potentially a useful addition, since it allows databases to always be connected. It would also be useful for planning rejoins, since those can always be done by merge tables along foreign key references.

```{r cgv, echo = FALSE, results='asis'}
gvc <- "digraph {
  rankdir = \"LR\"
...
if (requireNamespace("DiagrammeR", quietly = TRUE)) {
  cat("Mockup of how the schema might look:\n")
  DiagrammeR::grViz(gvc)
}
```

```{r cgv2, echo = FALSE, results='asis'}
gvc2 <- "digraph {
  rankdir = \"LR\"
  node [shape=plaintext];
...
if (requireNamespace("DiagrammeR", quietly = TRUE)) {
  cat("We could explicitly show all of the decomposition steps as views, but it's not clear that it's worth the bother:")
  DiagrammeR::grViz(gvc2)
}
```
...
The above schema is in third normal form. However, it is not in the higher Boyes-Codd normal form (BCNF), because `c -> a` is present in relation `a_b`, but not in terms of a key. This results in some redundancy, but also means that the two representation of `c -> a` can hold sets of data that aren't mutually coherent :
...
The resulting schema is now in BCNF, but we have a different problem: the dependency `{a, b} -> c` has disappeared! Fixing this can't be done with just real relations, but we can solve it by adding a virtual relation:
...
schema3 <- c(
  ...
  database_schema(
    relation_schema(
      list(view = list(letters[1:3], list(c("a", "b")))),
      letters[1:3]
    ),
    list()
  )
)
references(schema3) <- c(
  references(schema3),
  list(
    list("view", c("b", "c"), "b_c", c("b", "c")),
    list("view", "c", "c", "c")
  )
)
show(schema3)
