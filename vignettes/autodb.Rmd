...
The result is a list of functional dependencies, in the format `determinant set -> dependent`, with an attribute named `attrs` that gives all the attribute names in their original order. Each of these three parts has its own (generic) extraction function:
...
nudge_schema <- cross_reference(synthesise(nudge_deps, remove_avoidable = TRUE))
...
nudge_schema_filtered <- cross_reference(synthesise(nudge_deps_filtered, remove_avoidable = TRUE))
...
nudge_schema_filtered2 <- cross_reference(synthesise(nudge_deps_filtered2, remove_avoidable = TRUE))
...
avoid_deps
avoid_schema <- cross_reference(synthesise(avoid_deps))
...
We can have this removal of avoidable attributes done automatically, using the `remove_avoidable` flag for `synthesise`:

```{r avoid_remove}
avoid_schema_removed <- cross_reference(synthesise(
  avoid_deps,
