...
We can also plot it, by using `gv` to turn the result into code for the Graphviz language. In an interactive session, the simplest way to do this is with the DiagrammeR package:
...
We could also save the Graphviz code to a file, and either call Graphviz on it directly, or call it from a `dot` code block in Quarto. Unlike DiagrammeR, these don't create an HTML widget, so you get a static image instead of something built on top of about 2MB of Javascript.
