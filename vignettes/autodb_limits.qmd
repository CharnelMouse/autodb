---
title: "autodb limitations"
vignette: >
  %\VignetteIndexEntry{autodb_limits2}
...
database_schema(
  ...
  gv() |>
  write("citation_separate.gv")
```

```{dot}
//| file: citation_separate.gv
```
...
database_schema(
  ...
  gv() |>
  write("citation_joined.gv")
```

```{dot}
//| file: citation_joined.gv
```
...
```{r}
#| include: false
write(gv(schema_redkey), "redkey.gv")
```
...
```{r}
#| include: false
write(gv(schema_redkey_fix), "redkey_fix.gv")
```
...
```{r dup_example_dup}
#| include: false
write(gv(dup_db[c(1, 1, 2, 2, 2)]), "dup_dup.gv")
```
